...
theorem IsChain.imp_of_mem_of_mem_tail {S : α → α → Prop} {l : List α}
    (H : ∀ a b : α, a ∈ l → b ∈ l.tail → R a b → S a b) (p : IsChain R l) : IsChain S l := by
  induction p with grind

theorem IsChain.imp_of_mem {S : α → α → Prop} {l : List α}
    (H : ∀ a b : α, a ∈ l → b ∈ l → R a b → S a b) (p : IsChain R l) : IsChain S l :=
  p.imp_of_mem_of_mem_tail (H · · · <| mem_of_mem_tail ·)
...
theorem IsChain.iff_of_mem {S : α → α → Prop} {l : List α}
    (H : ∀ a b : α, a ∈ l → b ∈ l → (R a b ↔ S a b)) : IsChain R l ↔ IsChain S l :=
  ⟨IsChain.imp_of_mem (Iff.mp <| H · · · ·), IsChain.imp_of_mem (Iff.mpr <| H · · · ·)⟩

theorem IsChain.iff_of_mem_of_mem_tail {S : α → α → Prop} {l : List α}
    (H : ∀ a b : α, a ∈ l → b ∈ l.tail → (R a b ↔ S a b)) : IsChain R l ↔ IsChain S l :=
  ⟨IsChain.imp_of_mem_of_mem_tail (Iff.mp <| H · · · ·),
  IsChain.imp_of_mem_of_mem_tail (Iff.mpr <| H · · · ·)⟩

theorem IsChain.iff_mem : ∀ {l : List α}, IsChain R l ↔
    IsChain (fun x y => x ∈ l ∧ y ∈ l ∧ R x y) l := IsChain.iff_of_mem <| by grind

theorem IsChain.iff_mem_mem_tail : ∀ {l : List α}, IsChain R l ↔
    IsChain (fun x y => x ∈ l ∧ y ∈ l.tail ∧ R x y) l :=
  IsChain.iff_of_mem_of_mem_tail <| by grind
...
@[deprecated (since := "2025-09-19")]
alias Chain.sublist_cons := IsChain.sublist
...
@[deprecated (since := "2025-09-19")] alias chain_iff_get := isChain_iff_get
...
@[deprecated (since := "2025-09-19")]
alias Chain.iff := IsChain.iff

@[deprecated (since := "2025-09-19")]
alias Chain.iff_mem := IsChain.iff_mem_mem_tail
...
theorem IsChain.backwards_induction (p : α → Prop) (l : List α) (h : IsChain r l)
    ...
    ∀ i ∈ l, p i := by
  have H : IsChain (flip (flip r)) l := by simpa [IsChain]
...
theorem isChain_cons_eq_iff_eq_replicate {a : α} {l : List α} :
  ...
  rw [isChain_eq_iff_eq_replicate]
  simp only [head?_cons, Option.mem_def, Option.some.injEq, length_cons,
    replicate_succ, cons.injEq, forall_eq', true_and]
